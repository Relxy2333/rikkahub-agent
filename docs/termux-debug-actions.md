...
工作流先保存在 `fix/termux-result-newline-20260910` 修复分支。
本次只创建配置，不触发运行，也不修改默认分支。

GitHub 要求默认分支上存在工作流文件，才会接受手动触发事件。
因此后续首次启动前，需要把这份工作流文件单独添加到默认分支 `master`，
并确认 fork 已启用 Actions；这不要求把修复代码合并到 `master`。
相关说明见 [GitHub 手动运行工作流文档](https://docs.github.com/en/actions/how-tos/manage-workflow-runs/manually-run-a-workflow)。

完成上述准备后，在 Actions 中打开工作流并点击 **Run workflow**。
...
3. 安装 Android API 37、Build Tools 36.0.0、NDK 28.2.13676358 和 CMake 3.22.1。
